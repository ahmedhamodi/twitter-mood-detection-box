...
class Object(Model):
        ...
        if _ is not None:
            if isinstance(_, Object):
                ...
                if isinstance(_, IOBase):
                    if hasattr(_, 'url'):
                        serial.meta.writable(self).url = _.url
                    elif hasattr(_, 'name'):
                        serial.meta.writable(self).path = _.name
                _ = deserialize(_)
    ...
    def __setattr__(self, property_name, value):
        ...
        if property_name[0] != '_':
            try:
                property_definition = serial.meta.writable(self).properties[property_name]
                try:
    ...
    def __init__(
        ...
        if isinstance(items, IOBase):
            if hasattr(items, 'url'):
                serial.meta.writable(self).url = items.url
            elif hasattr(items, 'name'):
                serial.meta.writable(self).path = items.name
